...
1. 新建「空白 **C++** 项目」（EasyX 内部使用 C++ 特性，必须以 C++ 模式链接）
2. 将 `src/` 目录下全部 `.c` 文件加入项目
3. 项目属性 → **C/C++ → 高级 → 编译为：编译为 C++ 代码 (/TP)**
4. 项目属性 → **高级 → 字符集 → 使用多字节字符集**（中文显示必须）
5. 直接 F5 运行即可
...
> **编译选项说明**
> - `-x c++`：强制以 C++ 模式编译 `.c` 文件（EasyX `graphics.h` 包含 C++ 代码，`gcc` 无法编译）
> - `-fexec-charset=GBK`：将源文件（UTF-8）中的中文字符串字面量在编译时转换为 GBK，
>   使 EasyX 的 ANSI 绘图函数（`outtextxy` 等）在中文 Windows 上显示正常。
>   不加此选项将出现乱码。
...
> 驱动力必须同时满足扭矩和功率两条约束，否则极速计算将严重失真。
> 例：BYD 汉 380 kW / 700 N·m，不加功率限制时理论极速约 840 km/h。
...
4. 选「制动测试」→ 等仪表盘显示 100 km/h → **按空格**触发制动
5. 选「车型管理」→ 按 **A** 键演示键盘录入自定义车型
...
| fix: 全面修正 | 功率限制 / EasyX 输入 / Y 轴自动缩放 / 编译文档 |
